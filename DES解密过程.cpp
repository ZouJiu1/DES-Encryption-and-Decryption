...
public:
    ...
    int*fe(void)
    {
        ...
        if(count%64!=0)//bit流不是64的倍数
        {
            j=count/64;
            for(i=count;i<(j+1)*64;i++)
            {
                f[i]=0;
            }
            count=(j+1)*64;
...
//进行8字符为一段的循环加密
    for(th=0;th<(count/8);)
    {
        for(N=16;N>=1;N--)//进行16轮的迭代
        {
            ...
            {
                if(N==1)
                    R[th][17-N][i]=R[th][16-N][i];
                else
                    L[th][17-N][i]=R[th][16-N][i];
            }
            for(i=0;i<48;i++)//将32比特R扩展成48比特
            {
                EA[i]=R[th][16-N][E[i]-1];
            }
